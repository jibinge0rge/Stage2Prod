...
describe('GET /api/repos', () => {
  it('lists only active watched repos', async () => {
    ...
    await ctx.reposRepo.add('acme', 'archived');
    await ctx.reposRepo.remove('acme', 'archived');
...
describe('POST /api/repos', () => {
  ...
  it('adds a repo once GitHub confirms it is reachable, auto-filling productionBranch from GitHub\'s default branch', async () => {
    ...
    expect(await ctx.reposRepo.isActive('acme', 'widgets')).toBe(true);
  });
  ...
  it('400s when productionBranch and stagingBranch would be the same', async () => {
    ...
    expect(res.status).toBe(400);
    expect(await ctx.reposRepo.get('acme', 'widgets')).toBeNull();
  });
  ...
  it('404s and does not add the repo when GitHub reports it does not exist', async () => {
    ...
    expect(await ctx.reposRepo.get('acme', 'ghost')).toBeNull();
  });
});
...
describe('PATCH /api/repos/:owner/:name', () => {
  ...
  it('saves a custom lifecycle status map per repo', async () => {
    const ctx = await buildTestCtx();
    await ctx.reposRepo.add('acme', 'widgets', { jiraProjectKey: 'VIM' });
    const app = createApp(ctx);
  ...
  it('saves DE/QA/DA/CDL team roles per repo', async () => {
    ...
    expect((await ctx.reposRepo.get('acme', 'widgets')).teamRoles.de[0].displayName).toBe('Ada');
  });
  ...
  it('saves a default QA when several QA are configured', async () => {
    ...
    expect((await ctx.reposRepo.get('acme', 'widgets')).teamRoles.defaults.qa).toBe('q2');
  });
  ...
  it('400s when the update would make productionBranch equal stagingBranch', async () => {
    const ctx = await buildTestCtx();
    await ctx.reposRepo.add('acme', 'widgets', { productionBranch: 'develop', stagingBranch: 'staging' });
    const app = createApp(ctx);
    ...
    expect(res.status).toBe(400);
    expect((await ctx.reposRepo.get('acme', 'widgets')).productionBranch).toBe('develop');
  });
  ...
  it('reclassifies linked tickets when staging/production branches change', async () => {
    ...
    expect((await ctx.ticketsRepo.get('VIM-115')).pipeline_state).toBe('unmerged');
    expect((await ctx.ticketsRepo.get('VIM-115')).pr_number).toBeNull();
  });
});
...
describe('DELETE /api/repos/:owner/:name', () => {
  ...
  it('soft-removes a watched repo', async () => {
    ...
    expect(await ctx.reposRepo.isActive('acme', 'widgets')).toBe(false);
  });
});
