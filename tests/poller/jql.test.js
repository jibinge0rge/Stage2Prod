...
const config = { JIRA_JQL: 'project = PROJ AND status CHANGED AFTER -5m ORDER BY updated ASC', JIRA_POLL_CLAUSE: 'status CHANGED AFTER -5m' };
...
describe('buildPollJql', () => {
  ...
  it('builds a project-scoped query from a single configured repo', () => {
    expect(buildPollJql([repo('PROJ')], config)).toBe('project in ("PROJ") AND status CHANGED AFTER -5m ORDER BY updated ASC');
  });

  it('unions multiple distinct project keys across repos', () => {
    const result = buildPollJql([repo('PROJ'), repo('OTHER')], config);
    expect(result).toBe('project in ("PROJ", "OTHER") AND status CHANGED AFTER -5m ORDER BY updated ASC');
  });

  it('de-duplicates repos that share the same project key', () => {
    const result = buildPollJql([repo('PROJ'), repo('PROJ')], config);
    expect(result).toBe('project in ("PROJ") AND status CHANGED AFTER -5m ORDER BY updated ASC');
  });
